...
//クラスParticleSystemのオブジェクトpartSysを作成する
...
void setup() {
  ...
  //パーティクルシステムの実体化
  partSys = new ParticleSystem(new PVector(width/2, 50));
}
...
void draw() {
  ...
  //パーティクルをトラッキングの円の座標に描画する
  for (int i = 0; i < 3; i++) {
    //Class ParticleSystemのaddParticleを(lerpX, lerpY)の値を渡して実行する
    //ParticleSystemを経由してClass Particle（単一の粒を発生）を実行
    partSys.addParticle(lerpX, lerpY);
  }
  partSys.run();
}
